  ...
  return (
    <section id="the-calling" className="min-h-[100svh] relative flex items-center justify-center text-center bg-brand-cream px-4 sm:px-6 pt-32 pb-40 overflow-x-hidden z-0">
      ...
      <div className="relative z-10 max-w-5xl mx-auto flex flex-col items-center">
        ...
        {/* H1 Headline — "Guidance Path." */}
        <h1 ref={headingRef} className="text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-serif font-normal text-stone-800 mb-12 md:mb-16 leading-[1.1] md:leading-tight tracking-tight px-4 w-full">
          {'Guidance Path.'.split('').map((char, i) => (
            <span
              key={`c-${i}`}
        ...
        {/* Decorative Divider */}
        <div className={`w-32 h-[1px] bg-gradient-to-r from-transparent via-amber-600/20 to-transparent mb-16 ${isReady ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '700ms' }}></div>

        {/* Subheadline — "Your life begins to reorganize from within." */}
        <div className="max-w-3xl mx-auto mb-16 md:mb-24 px-6">
          <p className="text-stone-600 text-lg md:text-2xl font-serif font-normal leading-relaxed tracking-normal">
            {["Your", "life", "begins", "to", "reorganize", "from", "within."].map((word, i) => (
              <span
                key={`s1-${i}`}
