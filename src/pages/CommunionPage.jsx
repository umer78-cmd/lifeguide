    ...
    <div ref={containerRef} className="bg-brand-cream overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6 pt-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <motion.p 
            ...
            className="max-w-2xl mx-auto text-stone-600 text-lg md:text-xl font-light leading-relaxed tracking-wide"
          >
      ...
      <section id="paths" className="py-24 md:py-40 px-6 bg-brand-cream relative z-10">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20">
            ...
            {/* Path A: The commIN Space */}
            <motion.div 
              initial={{ opacity: 0, y: 40 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}
              className="group bg-white/40 backdrop-blur-sm p-8 md:p-16 rounded-[2.5rem] border border-stone-200/50 hover:shadow-2xl transition-all duration-700 hover:-translate-y-2 flex flex-col h-full"
            >
              <p className="text-brand-gold/60 text-xs uppercase tracking-brand-wide font-bold mb-6">Path A — The Entry</p>
              <h2 className="text-4xl md:text-5xl font-serif text-brand-stone mb-8">comm<span className="text-brand-gold">IN</span> Group Space</h2>
              <p className="text-stone-600 font-light text-lg mb-12 leading-relaxed italic">
              ...
              <div className="space-y-10 mb-16 flex-grow">
                {[
                  ...
                  <div key={i} className="flex gap-6 items-start">
                    <span className="text-brand-gold/30 font-serif text-2xl mt-1">0{i+1}</span>
                    <div>
                      <h3 className="text-xl font-serif text-brand-stone mb-1">{item.title}</h3>
                      <p className="text-stone-500 font-light text-sm uppercase tracking-widest">{item.subtitle}</p>
                    </div>
                  </div>
                ))}
              </div>

              <button className="w-full py-5 bg-stone-900 text-white uppercase text-xs tracking-brand-wide font-bold rounded-full hover:bg-brand-gold transition-colors duration-500 shadow-xl">
                Join the Space
              </button>
            </motion.div>

            {/* Path B: Holistic LifeGuide */}
            <motion.div 
              initial={{ opacity: 0, y: 40 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 1, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
              className="group bg-stone-950 p-8 md:p-16 rounded-[2.5rem] border border-stone-800 hover:shadow-[0_40px_100px_-20px_rgba(180,83,9,0.3)] transition-all duration-700 hover:-translate-y-2 flex flex-col h-full text-white"
            >
              <p className="text-brand-gold text-xs uppercase tracking-brand-wide font-bold mb-6">Path B — The Depth</p>
              <h2 className="text-4xl md:text-5xl font-serif mb-8">Individual Guidance Path</h2>
              <p className="text-stone-300 font-light text-lg mb-12 leading-relaxed italic">
                ...
                {[
                  ...
                  <div key={i} className="flex gap-6 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-stone-400 font-light text-sm uppercase tracking-widest">{item.subtitle}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-32 md:py-52 bg-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 items-center gap-20">
          ...
          >
            <p className="text-brand-gold text-sm uppercase tracking-brand-wide font-bold mb-8">The Methodology</p>
            ...
            <div className="mt-12 flex gap-12">
              <div>
                ...
                <p className="text-stone-400 text-xs uppercase mt-2 tracking-widest">Pressure</p>
              </div>
              <div className="w-px h-12 bg-stone-200"></div>
              <div>
                <p className="text-3xl font-serif text-brand-stone">100%</p>
                <p className="text-stone-400 text-xs uppercase mt-2 tracking-widest">Potential</p>
              </div>
            </div>
          </motion.div>
